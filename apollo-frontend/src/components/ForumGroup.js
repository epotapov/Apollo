...
const tagPair = { "Announcement": "magenta", "Post": "purple", "Education": "geekblue",
                  "Sports": "red", "Meetings": "green" }; 
...
const ForumGroup = (props) => {
    ...
    const handleEditComment = async () => {
        ...
        if (response.ok) {
            ...
            fetch("http://localhost:5001/api/thread/group/" + courseName)
    ...
    const handleCreateThread = async (values) => {
        ...
        const response = await fetch('http://localhost:5001/api/thread/createGroupThread', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
    ...
    return (
        <div>
            {isProf ? ( 
                <Title level={2}> Professor Forum for {title} </Title>
            ) : (
                <Title level={2}> Forum for {title} </Title>
            )}
            {threads.length === 0 ? (
                <div>
